...
class AccountProductControllerTest extends ControllerTest {
    ...
    public void get_products() throws Exception {
        List<AccountProductSummary> summaries = List.of(new AccountProductSummary(
                1L,
                "샘플 상품",
                1L,
                2.5,
    ...
    public void create_product() throws Exception {
        ...
        MvcResult result = mockMvc.perform(post("/api/v1/accountproducts")
                        .content(jsonRequest)
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isOk())
        ...
        AccountProductSummary response = objectMapper.readValue(
                result.getResponse().getContentAsString(),
